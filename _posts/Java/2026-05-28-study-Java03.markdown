...
- [Period 01 — 변수 100개 만들 거야?](#period-01--변수-100개-만들-거야)
  - [배열 선언, 그리고 메모리에서 일어나는 일](#배열-선언-그리고-메모리에서-일어나는-일)
  - [실습 1 — 점수 5개의 합계 구하기](#실습-1--점수-5개의-합계-구하기)
  - [실습 2 — 평균을 계산하라](#실습-2--평균을-계산하라)
  - [잠깐 — arr\[5\]는 6번째가 아니다](#잠깐--arr5는-6번째가-아니다)
  - [실습 3 — 거꾸로 출력 (반전을 적용)](#실습-3--거꾸로-출력-반전을-적용)
  - [도전 문제 — 짝수는 몇 개?](#도전-문제--짝수는-몇-개)
- [Period 02 — 1등은 누구?](#period-02--1등은-누구)
  - [“순회”에서 “알고리즘”으로 — 비교가 등장한다](#순회에서-알고리즘으로--비교가-등장한다)
  - [실습 1 — 최고점 출력](#실습-1--최고점-출력)
  - [실습 2 — 1등의 “이름”까지 출력하라](#실습-2--1등의-이름까지-출력하라)
  - [실습 3 — 선택 정렬 (Selection Sort)](#실습-3--선택-정렬-selection-sort)
  - [도전 문제 — 내림차순 정렬 후, 상위 3명 출력](#도전-문제--내림차순-정렬-후-상위-3명-출력)
- [Period 03 — 표를 만들자](#period-03--표를-만들자)
  - [학생 3명, 과목 3개 — 1차원으로 다루면?](#학생-3명-과목-3개--1차원으로-다루면)
  - [2차원 배열 선언과 메모리 구조](#2차원-배열-선언과-메모리-구조)
  - [실습 1 — 4 x 6 좌석표 출력](#실습-1--4-x-6-좌석표-출력)
  - [실습 2 — 학생 x 과목 평균](#실습-2--학생-x-과목-평균)
  - [실습 3 — 계단형(jagged) 별 찍기](#실습-3--계단형jagged-별-찍기)
  - [도전 문제 — 5x5 행렬의 두 대각선 합 구하기](#도전-문제--5x5-행렬의-두-대각선-합-구하기)
- [Period 04 — 같은 코드를 두 번 쓰지 마라](#period-04--같은-코드를-두-번-쓰지-마라)
  - [main이 점점 길어진다 — 절차지향의 한계](#main이-점점-길어진다--절차지향의-한계)
  - [메서드의 6가지 부품](#메서드의-6가지-부품)
  - [반환 타입과 return은 한 몸 — 어긋나면 컴파일러가 거절한다](#반환-타입과-return은-한-몸--어긋나면-컴파일러가-거절한다)
  - [도전 문제 — 소수 판별 메서드 boolean isPrime(int n)](#도전-문제--소수-판별-메서드-boolean-isprimeint-n)
- [Period 05 — 같은 이름, 다른 일](#period-05--같은-이름-다른-일)
  - [매개변수(Parameter) vs 인자(Argument) — 헷갈리지 마라](#매개변수parameter-vs-인자argument--헷갈리지-마라)
  - [오버로딩(Overloading) — 같은 이름, 다른 시그니처](#오버로딩overloading--같은-이름-다른-시그니처)
  - [실습 1 — print 메서드 3종 오버로딩](#실습-1--print-메서드-3종-오버로딩)
  - [실습 2 — avg 오버로딩 (2개 • 3개 • 배열)](#실습-2--avg-오버로딩-2개--3개--배열)
  - [컴파일러가 “모르겠다”고 손을 든다 — Ambiguous!](#컴파일러가-모르겠다고-손을-든다--ambiguous)
  - [도전 문제 — max 메서드 3종을 직접 만들고 모두 호출하라](#도전-문제--max-메서드-3종을-직접-만들고-모두-호출하라)
- [Period 06 — 메서드 호출의 비밀](#period-06--메서드-호출의-비밀)
  - [기본형 매개변수 (Primitive Parameter): 값이 복사된다](#기본형-매개변수-primitive-parameter-값이-복사된다)
  - [자바는 항상 값 전달입니다. 그 값이 참조일 뿐.](#자바는-항상-값-전달입니다-그-값이-참조일-뿐)
  - [도전 문제 — 두 사실을 출력으로 직접 증명하라](#도전-문제--두-사실을-출력으로-직접-증명하라)
- [Period 07 — 종합 실습 (학생 성적 관리 v0)](#period-07--종합-실습-학생-성적-관리-v0)
  - [도전 — 메뉴 “5) 학생 추가”를 만들어보라](#도전--메뉴-5-학생-추가를-만들어보라)
    - [Q1. 배열 길이가 고정이라 어떤 불편이 생겼나요?](#q1-배열-길이가-고정이라-어떤-불편이-생겼나요)
    - [Q2. 두 배열(names, scores)을 동시에 관리하는 게 왜 위험한가요?](#q2-두-배열names-scores을-동시에-관리하는-게-왜-위험한가요)
    - [Q3. 만약 “Student”라는 새 타입이 있다면 어떻게 달라질까요?](#q3-만약-student라는-새-타입이-있다면-어떻게-달라질까요)
